 ...
 * 
 * Onboards customers by identifying:
 * - Industry
 * - Offer type
 * - Sales motion
 * - Deal size
...
export const IndustryAdaptationEngine = ({ onComplete }: { onComplete?: () => void }) => {
  const { 
    setIndustry, 
    setOfferType, 
    setSalesMotion, 
            ...
            {step === 2 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl mx-auto">
                {salesMotions.map((motion) => (
                  ...
                  >
                    <h3 className="font-semibold mb-1">{motion.name}</h3>
            ...
            {step === 3 && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-3xl mx-auto">
                {dealSizes.map((size) => (
                  <button
                    key={size.id}
                    onClick={() => setSelectedDealSize(size.id as any)}
                    className={cn(
                      "p-5 rounded-lg border text-center transition-all duration-200",
