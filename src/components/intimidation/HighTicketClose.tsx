...
import { useIntimidationStore, highTicketLanguage } from '@/stores/intimidation-store';
import { cn } from '@/lib/utils';

/**
 * HIGH-TICKET CLOSE VARIANT™
 * 
 * Psychological certainty engineering for $10K-$250K+ deals.
 * Creates structural inevitability, not pressure.
 */

/**
 * OPPORTUNITY COST ESCALATOR
 * Shows money "lost" in real-time during evaluation
 */
export const OpportunityCostEscalator = () => {
  const { isHighTicketActive, highTicketActivatedAt, opportunityCostAccumulated, incrementOpportunityCost } = useIntimidationStore();
  const [displayCost, setDisplayCost] = useState(0);
  const intervalRef = useRef<NodeJS.Timeout | null>(null);

  useEffect(() => {
    if (isHighTicketActive && highTicketActivatedAt) {
      // Accumulate ~$47 per second (represents ~$141K/hour opportunity cost)
  ...
  // Smooth animation of displayed cost
  useEffect(() => {
  ...
  return (
    <motion.div
      ...
      className="fixed top-4 right-4 z-[60] p-4 rounded-lg bg-card/90 border border-warning/30 backdrop-blur-xl"
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[10px] text-muted-foreground">
            Opportunity cost since evaluation began
          </p>
        </div>
      </div>
    </motion.div>
...
/**
 * DECISION COLLAPSE VIEW
 * Compresses the decision into three undeniable facts
 */
export const DecisionCollapseView = () => {
  const { isHighTicketActive, closePhase } = useIntimidationStore();
  ...
  const facts = [
    {
      ...
      value: "Agencies, media buyers, content teams",
      detail: "$52,200/month in labor costs",
      color: "text-destructive"
    },
    {
      icon: CheckCircle2,
      label: "Being Gained",
      value: "Autonomous revenue operations",
      detail: "24/7 execution, compounding intelligence",
      color: "text-success"
    },
    {
      icon: Clock,
      label: "Compounding If You Wait",
      value: "Competitor advantage",
      detail: "Every day = missed learning cycles",
      color: "text-warning"
    }
  ];
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[55] bg-background/95 backdrop-blur-xl flex items-center justify-center p-8"
    >
      <div className="max-w-4xl w-full">
        <motion.p
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          className="text-xs font-mono text-primary uppercase tracking-[0.3em] text-center mb-8"
        >
        ...
        >
          This is not a menu. This is a system.
        </motion.p>
      </div>
    </motion.div>
  );
...
/**
 * OBJECTION PRE-NEUTRALIZATION PANEL
 * Surfaces answers before objections are voiced
 */
export const ObjectionNeutralizationPanel = () => {
  ...
  return (
    <motion.div
      ...
      className="fixed right-8 top-1/2 -translate-y-1/2 z-[55] w-80"
    >
      <p className="text-xs font-mono text-accent uppercase tracking-wider mb-4">
        Anticipated Concern #{visibleIndex + 1}
      </p>
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          exit={{ opacity: 0, y: -10 }}
          className="glass-card p-5 border-accent/20"
        >
 ...
 * Activation readiness, not purchase confirmation
 */
  ...
  return (
    ...
    >
      ...
      <div className="text-center max-w-lg">
        <motion.div
          animate={{ 
            scale: activationPulse ? 1.02 : 1,
            boxShadow: activationPulse 
        ...
        <motion.p
          ...
          className="text-xs font-mono text-primary uppercase tracking-[0.3em] mb-4"
        >
          System Status: Ready
        </motion.p>
      ...
      {/* Silence amplification - subtle background activity */}
 ...
 * UI continues updating even when founder stops talking
 * Creates discomfort with inactivity
 */
const SilenceAmplification = () => {
  const [metrics, setMetrics] = useState({
    learningCycles: 847,
    automationsQueued: 12,
...
/**
 * HIGH-TICKET TOGGLE
 * Stacks on Intimidation Mode
 */
export const HighTicketToggle = () => {
  const { isActive, isHighTicketActive, toggleHighTicket, closePhase } = useIntimidationStore();

  // Only show when Intimidation Mode is active
    ...
    >
      <div className="flex items-center gap-2">
        <DollarSign className={cn(
          "w-5 h-5",
          isHighTicketActive ? "text-warning" : "text-muted-foreground"
        )} />
        {isHighTicketActive && (
...
/**
 * CLOSE PHASE CONTROLLER
 */
export const ClosePhaseController = () => {
  const { isHighTicketActive, closePhase, setClosePhase } = useIntimidationStore();
