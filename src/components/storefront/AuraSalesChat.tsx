...
export function AuraSalesChat() {
      ...
      setMessages([
        {
          role: "assistant",
          content:
            "Hey there! ✨ Welcome to Aura Lift Essentials. I'm Aura, your personal beauty advisor. What are you looking for today? Skincare, beauty tech, or something specific?",
        },
      ]);
    }
  }, [messages.length]);
  ...
  const quickActions = [
    ...
    { label: "✨ Anti-aging", msg: "I'm looking for anti-aging products" },
      ...
      <AnimatePresence>
        {!isOpen && (
          ...
          >
            <button
              onClick={handleOpen}
              className="relative flex items-center justify-center w-16 h-16 rounded-full shadow-2xl bg-gradient-to-br from-[#8B6F4E] to-[#6B4F2E] text-white hover:scale-110 transition-transform duration-200"
            ...
            {showPulse && (
              <motion.div
                ...
                className="absolute bottom-20 right-0 bg-white rounded-2xl shadow-xl p-3 pr-6 min-w-[200px] border border-[#8B6F4E]/20"
              >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-[9999] w-[380px] max-w-[calc(100vw-2rem)] h-[560px] max-h-[calc(100vh-4rem)] flex flex-col bg-white rounded-2xl shadow-2xl border border-[#8B6F4E]/10 overflow-hidden"
          >
            {/* Header */}
                ...
                >
                  <div
                    className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
                      msg.role === "user"
                        ? "bg-[#8B6F4E] text-white rounded-br-sm"
